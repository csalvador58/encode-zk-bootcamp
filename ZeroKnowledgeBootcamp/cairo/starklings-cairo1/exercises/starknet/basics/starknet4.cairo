...
mod LizInventory {
    ...
    #[external]
    fn add_stock(product: felt252, qty: u32) {
        // TODO:
        ...
        let quantity = storage::read(product);
        assert(owner == caller, 'Caller is not the owner');
        if owner == caller {
            storage::write(product, quantity + qty);
        }
    }

    #[external]
    fn purchase(product: felt252, qty: u32) {
        // TODO:
        ...
        let stock = storage::read(product);
        assert(stock >= qty, 'Product not in stock');
        if stock >= qty {
            storage::write(product, stock - qty);
        }
